## 1. パイプライン: confidence・websitesの取得
...
## 2. パイプライン: GeoJSON propertiesへの反映
...
## 4. ビューア: ズーム連動confidenceフィルタ

- [ ] 4.1 `web/src/main.js`のカフェレイヤ(`CAFE_LAYER_ID`)に、`["step", ["zoom"], 0.99, 15, 0.97, 16, 0.95, 17, 0.90]`としきい値比較を組み合わせた`filter`式を追加する(design.md Decision 3)。
- [ ] 4.2 z10-14/z15/z16/z17以上の各ズーム帯でconfidenceしきい値未満のPOIが表示されないことを、ローカルサーバー上のブラウザ確認で検証する。

## 5. ビューア: ナビゲーションコントロールとURLハッシュ
...
## 7. ビューア: ポップアップへのconfidence・websites追加
