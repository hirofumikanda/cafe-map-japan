...
- [ ] 2.5 全国分を取得して生成されたGeoJSONの件数・プロパティ内容を確認する (未実施: 開発サンドボックスではNodeプロセスからのOverpass APIへの直接アウトバウンド接続が制限されており、`npm run fetch`によるnode実行では接続できなかった。curl経由では同一クエリ・同一変換ロジックの正当性を実データで確認済み。詳細はPR本文参照。ネットワーク制限のないCI/実行環境での全国分実行はフォローアップとする)
