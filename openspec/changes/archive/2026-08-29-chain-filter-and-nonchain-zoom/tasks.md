...
- [x] 5.1 `cd web && npm run build` で `src/` の変更が `public/` に反映されることを確認する。
- [x] 5.2 `cd web && npm test` で全テストが通ることを確認する。
- [x] 5.3 ローカル配信(`web/README.md` 参照)で、プルダウン「すべて」時に z10〜13 ではチェーン店のみ、z14 以上でチェーン店以外も表示されることをブラウザで確認する。
- [x] 5.4 特定チェーン(例: スターバックス コーヒー)選択時に当該チェーンのPOIのみ表示され、z10〜13 でも表示されること、「すべて」に戻すと元の条件に戻ることを確認する。
- [x] 5.5 プルダウンが地図左上に表示され、PC(クリック)・スマホエミュレーション(タッチ)双方で開閉・選択でき、地図のドラッグ/ズームを妨げないことを確認する。
- [x] 5.6 いずれの選択状態でも confidence しきい値フィルタが維持されている(z15 で 0.97 未満が出ない)ことを確認する。
